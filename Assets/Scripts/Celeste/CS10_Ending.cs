...
namespace Celeste
{
    public class CS10_Ending : CutsceneEntity
    {
        ...
        private IEnumerator Cutscene(Level level)
        {
            ...
            cs10Ending.Add(cs10Ending.attachment = new Image(cs10Ending.Atlas["21-window"]));
            cs10Ending.Add(cs10Ending.picture = new Image(cs10Ending.Atlas["21-picture"]));
            cs10Ending.Add(cs10Ending.ok = new Image(cs10Ending.Atlas["21-button"]));
            cs10Ending.Add(cs10Ending.cursor = new Image(cs10Ending.Atlas["21-cursor"]));
            ...
            yield return 1f;
            yield return Textbox.Say("CH9_END_CINEMATIC", cs10Ending.ShowPicture);
            ...
            for (float p = 0.0f; p < 1.0; p += Engine.DeltaTime / 4f)
            {
                ...
                cs10Ending.picture.Scale = Vector2.One * (float) (0.89999997615814209 + 0.10000002384185791 * p);
                yield return null;
            }
        ...
        private IEnumerator ShowPicture()
        {
            ...
            float p;
            for (p = 0.0f; p < 1.0; p += Engine.DeltaTime / 0.3f)
            {
                cs10Ending.attachment.Scale.Y = (float) (0.25 + 0.75 * Ease.BigBackOut(p));
                cs10Ending.attachment.Scale.X = (float) (1.5 - 0.5 * Ease.BigBackOut(p));
                yield return null;
            }
            yield return 0.25f;
            ...
            for (p = 0.0f; p < 1.0; p += Engine.DeltaTime / 0.25f)
            {
                cs10Ending.ok.Scale.Y = (float) (0.25 + 0.75 * Ease.BigBackOut(p));
                cs10Ending.ok.Scale.X = (float) (1.5 - 0.5 * Ease.BigBackOut(p));
                yield return null;
            }
            yield return 0.8f;
            ...
            for (p = 0.0f; p < 1.0; p += Engine.DeltaTime / 0.4f)
            {
                cs10Ending.picture.Scale.Y = (float) ((0.89999997615814209 + 0.10000000149011612 * Ease.BigBackOut(p)) * 0.89999997615814209);
                cs10Ending.picture.Scale.X = (float) ((1.1000000238418579 - 0.10000000149011612 * Ease.BigBackOut(p)) * 0.89999997615814209);
        ...
        public override void Render()
        {
            ...
            if (Atlas != null && Frames != null && this.frame < Frames.Count)
            {
        ...
        private List<int> GetFrameData(string data)
        {
            ...
            for (int index1 = 0; index1 < strArray1.Length; ++index1)
            {
                ...
                else if (strArray1[index1].Contains('-'))
                {
        ...
        private IEnumerator Play(string data) => Play(GetFrameData(data));
        ...
        private IEnumerator Loop(string data, float duration = -1f)
        {
            ...
            float time = 0.0f;
            while (time < (double) duration || duration < 0.0)
            {
                frame = frames[(int) (time / 0.0833333358168602) % frames.Count];
