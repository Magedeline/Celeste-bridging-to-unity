...
namespace Celeste
{
    public class CompleteRenderer : HiresRenderer, IDisposable
    {
            ...
            if (xml != null)
            {
                ...
                foreach (object obj in xml["layers"])
                {
                    if (obj is XmlElement)
                    {
                        ...
                        if (xml1.Name == "layer")
                            Layers.Add(new ImageLayer(Offset, atlas, xml1));
                        else if (xml1.Name == "ui")
                        {
                            HasUI = true;
                            Layers.Add(new UILayer(this, xml1));
                        }
                    }
                }
            }
        ...
        {
            yield return delay;
            for (float p = 0.0f; p < 1.0; p += Engine.DeltaTime / SlideDuration)
            {
                yield return null;
        ...
        public class ImageLayer : Layer
        {
            ...
            public override void Render(Vector2 scroll)
            {
                ...
                Draw.SpriteBatch.Draw(image.Texture.Texture, position, rectangle, Color.White * Alpha, 0.0f, Vector2.Zero, Scale, flag ? SpriteEffects.FlipHorizontally : SpriteEffects.None, 0.0f);
                if (num == 0)
                    return;
                HiresRenderer.EndRender();
                HiresRenderer.BeginRender(BlendState.AlphaBlend, SamplerState.LinearClamp);
