...
namespace Celeste
{
    public class LightningBreakerBox : Solid
    {
        ...
        public DashCollisionResults Dashed(Player player, Vector2 dir)
        {
            ...
            sprite.Scale = new Vector2((float) (1.0 + Math.Abs(dir.Y) * 0.40000000596046448 - Math.Abs(dir.X) * 0.40000000596046448), (float) (1.0 + Math.Abs(dir.X) * 0.40000000596046448 - Math.Abs(dir.Y) * 0.40000000596046448));
            --health;
            if (health > 0)
            {
        ...
        public override void Update()
        {
            ...
            if (Collidable)
            {
                ...
                start.Y += (float) (sink * 6.0 + sine.Value * (double) MathHelper.Lerp(4f, 2f, sink));
